package view.panel
{
	...
	import service.command.user.ExtendFarmLandCommand;
	...
	public class ExtendFarmLandPanel extends PanelScreen
	{
		...
		public function ExtendFarmLandPanel()
		{
		...
		protected function initializeHandler(event:Event):void
		{
			...
			spec = SpecController.instance.getItemSpec(String(player.crop_extend + 41000 +1)) as ExtendSpec;
			if(!spec){
				Command.errorRefreh();
				return;
			}
			...
			var nameText:TextField = FieldController.createSingleLineDynamicField(panelwidth *.8,40*scale,LanguageController.getInstance().getString("buyland"),0x000000,35,true);
			...
			var icon:Image = new Image(Game.assets.getTexture("plowIcon"));
			addChild(icon);
			icon.width = icon.height =100*scale;
			icon.x = panelwidth*0.35-icon.width/2;
			icon.y = skin2.y +10*scale;
			
			var currentT:String = String(player.wholeFarmLand );
			...
			var nextT:String = String(player.wholeFarmLand+2);
		...
		private function coinButton_triggeredHandler(e:Event):void
		{
			if(!isCommanding){
				if(player.coin>= spec.coinPrice){
					new ExtendFarmLandCommand(Configrations.METHOD_COIN,onBuyCoin);
					isCommanding = true;
				}else{
		...
		private function gemButton_triggeredHandler(e:Event):void
		{
			if(!isCommanding){
				if(player.gem>= spec.gemPrice){
					new ExtendFarmLandCommand(Configrations.METHOD_MONEY,onBuy);
					isCommanding = true;
				}else{
					DialogController.instance.showPanel(new TreasurePanel);
					destroy();
