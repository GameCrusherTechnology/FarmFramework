package view.panel
{
	...
	import service.command.user.ExtendFarmCommand;
	...
	public class ExtendFarmPanel extends PanelScreen
	{
		...
		public function ExtendFarmPanel()
		{
		...
		protected function initializeHandler(event:Event):void
		{
			...
			spec = SpecController.instance.getItemSpec(String(player.extend + 40000 +1)) as ExtendSpec;
			if(!spec){
				Command.errorRefreh();
				return;
			}
			...
			var nameText:TextField = FieldController.createSingleLineDynamicField(panelwidth *.8,40*scale,LanguageController.getInstance().getString("buyscene"),0x000000,35,true);
			...
			var icon:Image = new Image(Game.assets.getTexture("extendIcon"));
			addChild(icon);
			icon.width = icon.height =100*scale;
			icon.x = panelwidth*0.35-icon.width/2;
			icon.y = skin2.y +10*scale;
			
			var currentT:String = player.wholeSceneLength +"×"+ player.wholeSceneLength;
			...
			arricon.x = icon.x +icon.width + 40*scale;
			arricon.y = icon.y +60*scale; 
			
			var nextT:String = (player.wholeSceneLength+2) +"×"+ (player.wholeSceneLength+2);
		...
		private function coinButton_triggeredHandler(e:Event):void
		{
			if(!isCommanding){
				if(player.coin>= spec.coinPrice){
					new ExtendFarmCommand(Configrations.METHOD_COIN,onBuyCoin);
					isCommanding = true;
				}else{
		...
		private function gemButton_triggeredHandler(e:Event):void
		{
			if(!isCommanding){
				if(player.gem>= spec.gemPrice){
					new ExtendFarmCommand(Configrations.METHOD_MONEY,onBuy);
					isCommanding = true;
				}else{
					DialogController.instance.showPanel(new TreasurePanel);
					destroy();
		...
		private function onBuyCoin():void
		{
			...
			destroy();
			GameController.instance.LoginFarm();
		}
